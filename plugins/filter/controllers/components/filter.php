...
class FilterComponent extends Object {
	 ...
	 * @var string
	 */
	var $actions = array('index','admin_index');
	...
	function processAction(&$controller){
		if (isset($controller->data['reset']) || isset($controller->data['cancel'])) {
			...
			$controller->redirect(array('action' => $controller->action, 'admin' => $controller->params[$admin]));
			return;
		}
	...
	function processFilters(&$controller){
		...
		/**
		 * Edit to remove filtering for "field==0" from Products index (for when an empty checkbox is filtered against).
		 */
		/*if($controller->name == 'Products'
			&& $controller->action == 'index'
			&& isset($controller->data['Product'])) {
			foreach($controller->data['Product'] as $key => $value) {
				if($value == 0) {
					unset($controller->data['Product'][$key]);
				}
			}
		} // Job done.*/
